...
impl SplitRules {
    ...
    /// - `?column` subst must appear in either searcher or chain if `apply_default_transform` is `true`;
    ...
    ///
    /// The last parameter `apply_default_transform` controls whether column check should be initiated.
    /// This will generally be `true` for rules that directly extract column, and `false` for those
    /// that simply unwrap the expression (for instance, extracting expr from a function).
                ...
                rules.extend(
                    vec![
                        ...
                        rewrite(
                            ...
                            agg_fun_expr(
                                "?output_fun",
                                vec![applier(|expr, _| expr.to_string())],
                                "?distinct",
                            ),
                        ),
                    ]
